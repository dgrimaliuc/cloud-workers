...
const verifyWhitelist = (_, payload) => {
  ...
  if (regexAllowedOrigin.some(r => r.test(origin))) {
    return true;
  } else return originWhitelist.length === 0 || originWhitelist.includes(origin);
};

const verifyResourceHost = (_, payload) => {
  payload.error = `The resource host is not allowed: `;
  return allowedResourceHosts.length === 0 || allowedResourceHosts.includes(payload.location.host);
};
